...
function Jobs() {
  ...
  const handleApply = (id) => {
    axios.post(`http://localhost:5000/api/endpoints/insertProviderToClient/${clientID}`, { clientID });
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/endpoints/getPosts');
        if (!response.ok) {
